...
public class State {
	...
	/**
	 * 	 Generates pseudo-legal moves of all pieces except king. Takes into consideration absolute pins. Moves generated by this method are legal
	 * 	 provided that the king of the side taking turn was not in check.
	 */
	private void generatePseudoLegalMovesExcludingKing(List<State> ouputMoves, int countOfPiecesTakingTurn,
													   Square[] squaresWithPiecesTakingTurn) {
		for (int i = 1; i < countOfPiecesTakingTurn; i++) {
			Square currSquare = squaresWithPiecesTakingTurn[i];
			Content piece = Content.fromShort(board[currSquare.ordinal()]);
			switch (piece) {
				case WHITE_PAWN:
				case BLACK_PAWN:
					generateLegalPawnMoves(currSquare, ouputMoves);
					break;
				case WHITE_KNIGHT:
				case BLACK_KNIGHT:
					generatePseudoLegalKnightMoves(currSquare, ouputMoves);
					break;
				case WHITE_BISHOP:
				case BLACK_BISHOP:
					generatePseudoLegalBishopMoves(currSquare, ouputMoves);
					break;
				case WHITE_ROOK:
				case BLACK_ROOK:
					generatePseudoLegalRookMoves(currSquare, ouputMoves);
					break;
				case WHITE_QUEEN:
				case BLACK_QUEEN:
					generatePseudoLegalQueenMoves(currSquare, ouputMoves);
					break;
				default:
					assert false : "Thing on:" + squaresWithPiecesTakingTurn[i] + " is unknown piece: " + piece;
			}
	...
	private void generatePseudoLegalQueenMoves(Square from, List<State> outputMoves) {
		generatePseudoLegalRookMoves(from, outputMoves);
		generatePseudoLegalBishopMoves(from, outputMoves);
	}

	private void generatePseudoLegalRookMoves(Square from, List<State> outputMoves) {
	...
	private void generatePseudoLegalBishopMoves(Square from, List<State> outputMoves) {
	...
	private void generateLegalMovesWhenKingInCheck(List<State> outputMoves, Square checkedKing,
		...
		if (getChecksCount(checkedKing, !test(WHITE_TURN)) < 2) {
			...
			generatePseudoLegalMovesExcludingKing(pseudoLegalMoves, countOfPiecesTakingTurn, squaresWithPiecesTakingTurn);
		...
		generateLegalKingMoves(checkedKing, outputMoves);
	}
